...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const dynamicSections = [
        ...
        },
        {
            ...
            title: 'One Pager',
            condition: (data) => {
                // Show if status is SUP_APPROVED (pending client agreement) or later stages
                const status = (data.final_status || data.supervisor_approval_status || '').toUpperCase();
                return ['SUP_APPROVED', 'CLIENT_APPROVED', 'WITH_UW', 'POLICY_CREATED'].includes(status);
            },
            render: async (contentDiv, uniqueId) => {
                // Load HTML
                try {
                    ...
                    if (resp.ok) {
                        ...
                        if (typeof window.renderOnePager === 'function') {
                            window.renderOnePager(contentDiv, uniqueId);
                        }
                    } else {
